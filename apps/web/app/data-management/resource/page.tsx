...
const AutocompleteInput = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      if (activeIndex >= 0 && filteredOptions[activeIndex]) handleSelect(filteredOptions[activeIndex]);
          ...
          {filteredOptions.length > 0 ? (
            filteredOptions.map((opt, idx) => (
              <li
                ...
                className={`px-4 py-2 text-[14px] text-[#1B1C1F] cursor-pointer transition-colors font-medium ${idx === activeIndex ? "bg-[#EBF0FF] text-[#21409A]" : "hover:bg-[#F2F4F7]"}`}
              >
                {opt}
              </li>
            ))
...
const FILTER_GRID: { key: keyof Resource | "year_range"; label: string; optionKey?: keyof ResourceFilterOptionsResponse; placeholder?: string }[] = [
  ...
  { key: "project_name", label: "Project Name", optionKey: "project_name", placeholder: "Select Project Name" },
  { key: "location", label: "Lokasi", optionKey: "location", placeholder: "Input Lokasi" },
  { key: "year_range", label: "Tahun", optionKey: "year" },
];
...
export default function ResourcesPage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([resourceApi.list(), resourceApi.filterOptions(), dashboardApi.getDashboard()])
      .then(([res, options, dashboard]) => {
        setAllResources(res.data);
        setFilterOptions(options);
        setHarsatTrend(dashboard.harsat_trend);
      })
      .catch(console.error)
  ...
  const handleSearch = () => {
    setLoading(true);
    setResources(filterResources(filters));
    setSearchApplied(true);
    setSnackbar(true);
    setLoading(false);
  };
