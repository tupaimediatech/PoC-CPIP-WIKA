...
function InfoRow({ label, value, valueClass }: { label: string; value: string | null; valueClass?: string }) {
  return (
    <div className="p-3 border border-gray-100 rounded-lg mb-3 last:mb-0">
      <p className="text-[14px] font-bold text-[#1B1C1F] mb-2">{label}</p>
      <p className={`text-[16px] text-gray-600 ${valueClass ?? ""}`}>{value ?? "-"}</p>
    </div>
  );
}
  ...
  return (
    <div className="bg-white min-h-screen" style={{ padding: "24px 32px" }}>
      ...
      <div className="bg-[#F9FAFB] rounded-xl p-5 mb-8">
        <p className="text-[14px] text-gray-600 leading-relaxed">{material.catatanMonitoring ?? "Tidak ada catatan monitoring."}</p>
      </div>
