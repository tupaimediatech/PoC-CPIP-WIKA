...
export default function CpiOverviewPage() {
  ...
  const getStatus = (cpiValue: string) => {
    const val = parseFloat(cpiValue);
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {paginated.map((item, index) => {
                const status = getStatus(item.cpi);
                return (
                  <tr
                    key={index}
                  ...
                  >
                    ...
                    <td className="px-6 py-[14px] text-[14px] text-gray-900 font-semibold">{parseFloat(item.cpi).toFixed(2)}</td>
