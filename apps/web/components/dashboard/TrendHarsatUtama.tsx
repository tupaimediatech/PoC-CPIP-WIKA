...
interface Tooltip {
  ...
  category: string; year: string; value: number; color: string;
}
...
export default function TrendHarsatUtama() {
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0">
      ...
      <div ref={chartRef} className="relative bg-white border border-gray-100 rounded-2xl" style={{ height: '380px', padding: '24px 16px 30px 16px' }}>
          ...
          <>
            ...
            <div className="flex gap-2 h-full">
              <div className="relative shrink-0" style={{ width: '28px', height: `${svgH}px` }}>
                {Y_TICKS.map(tick => (
                  <span key={tick} className="absolute text-[11px] text-gray-400 font-medium leading-none"
              ...
              <div className="flex-1 flex flex-col">
                    ...
                    {CATEGORIES.map(cat => {
                      ...
                      return (
                        <g key={cat.key}>
                          ...
                          <path d={linePath} fill="none" stroke="transparent" strokeWidth="16" className="cursor-pointer"
                ...
                <div className="flex justify-between pt-3">
                  {YEARS.map(year => (
                    <span key={year} className="text-[10px] font-medium text-gray-400">{year}</span>
                  ))}
                </div>
              </div>
            </div>
