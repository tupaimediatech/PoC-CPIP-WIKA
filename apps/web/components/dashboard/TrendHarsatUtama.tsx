...
export default function TrendHarsatUtama() {
  ...
  const [tooltip, setTooltip] = useState<Tooltip>({
    visible: false, x: 0, y: 0, category: '', year: '', value: 0, color: '',
  });
  function showTooltip(e: React.MouseEvent, category: string, color: string) {
    ...
    const catData = MOCK_DATA[CATEGORIES.find(c => c.label === category)!.key];
    setTooltip({ visible: true, x: e.clientX - rect.left, y: e.clientY - rect.top, category, year: YEARS[idx], value: catData[idx], color });
  }

  function moveTooltip(e: React.MouseEvent, category: string, color: string) {
    showTooltip(e, category, color);
  }
  ...
  function getX(idx: number) {
    return PADDING.left + (idx / (YEARS.length - 1)) * chartW;
  }
  function getY(val: number) {
    return PADDING.top + chartH - (val / MAX_VALUE) * chartH;
  }
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0">
      ...
      <div
        ...
        style={{ height: '380px', padding: '24px 16px 30px 16px' }}
      >
        <p className="text-[11px] text-gray-400 font-medium mb-2">IDR (Jillion)</p>
          ...
          <div className="flex-1 flex flex-col">
            <div className="relative" style={{ height: `${svgH}px` }}>
              ...
              >
                ...
                {CATEGORIES.map(cat => {
                  const values = MOCK_DATA[cat.key];
                  ...
                  return (
                    <g key={cat.key}>
                      ...
                      <path
                        ...
                        onMouseMove={(e) => moveTooltip(e, cat.label, cat.color)}
