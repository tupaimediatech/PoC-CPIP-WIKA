  ...
  return (
    <div className="flex flex-col gap-4 bg-white w-full" style={{ padding: "24px 32px" }}>
      <div className="flex items-center justify-between">
        <h2 className="text-[18px] font-bold text-[#1B1C1F]">Project List</h2>
          ...
          <tbody className="divide-y divide-gray-50">
            {paginatedProjects.map((project, index) => (
              ...
              >
                ...
                <td className="px-4 py-4 text-[14px] text-gray-700">{project.division}</td>
                <td className="px-4 py-4 text-[14px] text-gray-700 font-medium text-right">{formatCurrency(project.contract_value)}</td>
                <td className={`px-4 py-4 text-[14px] font-bold text-right ${kpiColor(project.cpi)}`}>{formatKpi(project.cpi)}</td>
                <td className={`px-4 py-4 text-[14px] font-bold text-right ${kpiColor(project.spi)}`}>{formatKpi(project.spi)}</td>
                <td className="px-6 py-4">{statusBadge(project.status)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {paginatedProjects.length === 0 && <div className="py-12 text-center text-gray-400 text-[14px]">No projects found.</div>}
      </div>
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={`px-3 py-1.5 text-[13px] rounded-md transition-colors ${
                  currentPage === page ? "bg-primary-blue text-white font-bold" : "text-gray-500 border border-gray-200 hover:bg-gray-50"
                }`}
              >
                {page}
              </button>
