...
class CompleteProjectSeeder extends Seeder
{
     ...
     * - 2 ProjectPeriods
     * - 4 ProjectWorkItems (2 per period)
     * - 4 ProjectMaterialLogs (2 per period)
     * - 4 ProjectEquipmentLogs (2 per period)
    ...
    public function run(): void
    {
        ...
        // ============================================================
        // LEVEL 2: PROJECT PERIODS (2 records)
        // ============================================================
        $periods = [];

        // Period 1: January 2024
        $periods[] = ProjectPeriod::updateOrCreate(
            [
        ...
        $this->command->info("✅ Created 2 ProjectPeriods");

        // ============================================================
        // LEVEL 3: PROJECT WORK ITEMS (2 per period = 4 total)
        // ============================================================
        foreach ($periods as $periodIndex => $period) {
            ...
            $category1 = ProjectWorkItem::updateOrCreate(
                ...
                [
                    ...
                    'realisasi'    => $periodIndex === 0 ? 25000000000 : 48000000000,
                    'deviasi'      => $periodIndex === 0 ? 25000000000 : 2000000000,
                    'deviasi_pct'  => $periodIndex === 0 ? 50.0000 : 4.0000,
                    'is_total_row' => false,
                ]
            );
            ...
            ProjectWorkItem::updateOrCreate(
                ...
                [
                    ...
                    'realisasi'    => $periodIndex === 0 ? 8000000000 : 14500000000,
                    'deviasi'      => $periodIndex === 0 ? 7000000000 : 500000000,
                    'deviasi_pct'  => $periodIndex === 0 ? 46.6667 : 3.3333,
                    'is_total_row' => false,
                ]
            );
            ...
            $category2 = ProjectWorkItem::updateOrCreate(
                ...
                [
                    ...
                    'realisasi'    => $periodIndex === 0 ? 40000000000 : 108000000000,
                    'deviasi'      => $periodIndex === 0 ? 330000000000 : 262000000000,
                    'deviasi_pct'  => $periodIndex === 0 ? 89.1892 : 70.8108,
                    'is_total_row' => false,
                ]
            );
            ...
            ProjectWorkItem::updateOrCreate(
                ...
                [
                    ...
                    'realisasi'    => $periodIndex === 0 ? 20000000000 : 55000000000,
                    'deviasi'      => $periodIndex === 0 ? 170000000000 : 135000000000,
                    'deviasi_pct'  => $periodIndex === 0 ? 89.4737 : 71.0526,
                    'is_total_row' => false,
                ]
            );
        }

        $this->command->info("✅ Created 4 ProjectWorkItems (2 per period)");

        // ============================================================
        // LEVEL 4A: PROJECT MATERIAL LOGS (2 per period = 4 total)
        // ============================================================
        foreach ($periods as $periodIndex => $period) {
            // Material Log 1
            ...
            ProjectMaterialLog::updateOrCreate(
                ...
                [
                    ...
                    'rating_performa'         => '4/5',
                    'qty'                     => $periodIndex === 0 ? 25.7500 : 48.5000,
                    'satuan'                  => 'ton',
                    'harga_satuan'            => 14500000.00,
                    'total_tagihan'           => $periodIndex === 0 ? 373375000.00 : 703250000.00,
                    'realisasi_pengiriman'    => '100% (Selesai)',
                    'deviasi_harga_market'    => '-2%',
                    'catatan_monitoring'      => 'Harga di bawah pasar, kualitas standar',
                    'is_discount'             => false,
                    'source_row'              => $periodIndex + 2,
                ]
            );
        }

        $this->command->info("✅ Created 4 ProjectMaterialLogs (2 per period)");

        // ============================================================
        // LEVEL 4B: PROJECT EQUIPMENT LOGS (2 per period = 4 total)
        // ============================================================
        foreach ($periods as $periodIndex => $period) {
            // Equipment Log 1
            ProjectEquipmentLog::updateOrCreate(
                ...
                [
                    'work_item_id'   => null,
                    'jam_kerja'      => $periodIndex === 0 ? 180.50 : 320.00,
                    'rate_per_jam'   => 450000.00,
                    'total_biaya'    => $periodIndex === 0 ? 81225000.00 : 144000000.00,
                    'payment_status' => 'Paid',
                    'source_row'     => $periodIndex + 1,
                ]
            );
            ...
            ProjectEquipmentLog::updateOrCreate(
                ...
                [
                    'work_item_id'   => null,
                    'jam_kerja'      => $periodIndex === 0 ? 95.00 : 175.50,
                    'rate_per_jam'   => 750000.00,
                    'total_biaya'    => $periodIndex === 0 ? 71250000.00 : 131625000.00,
                    'payment_status' => $periodIndex === 0 ? 'Paid' : 'Pending',
                    'source_row'     => $periodIndex + 2,
                ]
            );
        }

        $this->command->info("✅ Created 4 ProjectEquipmentLogs (2 per period)");
...
3. Pre-order material untuk phase berikutnya',
        ...
        $this->command->info('║  • Periods:          2                                  ║');
        $this->command->info('║  • Work Items:       4 (2 per period)                  ║');
        $this->command->info('║  • Material Logs:    4 (2 per period)                  ║');
        $this->command->info('║  • Equipment Logs:   4 (2 per period)                  ║');
